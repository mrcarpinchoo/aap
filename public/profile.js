...
async function fetchUserPets() {
    ...
    const res = await fetch(
        `http://localhost:4000/api/pets?ownerEmail=${userEmail}`,
        {
            method: 'GET',
            headers: {
...
document.addEventListener('DOMContentLoaded', function () {
                ...
                if (result.isConfirmed) {
                    try {
                        const response = await fetch(
                            `http://localhost:4000/api/pets/${uuid}`,
                            {
                                method: 'DELETE',
                                headers: {
                                    'Content-Type': 'application/json',
    ...
    document.addEventListener('click', function (event) {
        if (event.target.classList.contains('edit-pet')) {
            ...
            fetch(`http://localhost:4000/api/pets/${uuid}`)
    ...
    addPetForm.addEventListener('submit', async function (event) {
        ...
        try {
            ...
            const imageResponse = await fetch(
                `http://localhost:4000/api/images/${uuid}`,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
